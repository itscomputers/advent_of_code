...
describe Algorithms::Djikstra do
  #       2           3           5             9
  #  a   --->    b   --->    c   --->    d    <----    f
  #  |           |           |
  #  | 20        | 10        | 4
  ...
  describe "no initial target" do
    before { djikstra.search }

    it "has gives all shortest paths" do
      expected_paths.each do |target, expected_path|
        expect(djikstra.get_path(target: target)).to eq expected_path
        expect(djikstra.get_distance(target: target)).to eq expected_distances[target]
      end
    end

    it "has no path for f" do
      expect(djikstra.get_path(target: :f)).to be_nil
      expect(djikstra.get_distance(target: :f)).to be_nil
    end
  end
